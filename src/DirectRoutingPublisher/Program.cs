...
public  class Program
{
    ...
    {
        #region  payment and purchase order data
    ...
    private static void CreateConnection()
    {
        ...
        _model.ExchangeDeclare(exchange:ExchangeName, ExchangeType.Direct);
        _model.QueueDeclare(queue: CardPaymentQueueName, true, false, false, null);
        _model.QueueDeclare(queue: PurchaseOrderQueueName, true, false, false, null);

        _model.QueueBind(queue: CardPaymentQueueName, exchange: ExchangeName, routingKey: "CardPayment");
        _model.QueueBind(queue: PurchaseOrderQueueName, exchange: ExchangeName, routingKey: "PurchaseOrder");
    }

    private static void SendMessage(byte[] message, string routingKey)
    {                       
        _model.BasicPublish(exchange: ExchangeName, routingKey: routingKey, null, message);          
    }
}